...
\title{Análisis de interacciones de partículas mediante implementación de una librería para su detección con sensores CMOS}
  ...
  \renewcommand{\maketitlehookd}{
    \begin{abstract}\noindent
      Se detectaron interacciones de sensores CMOS-APS comerciales y partículas.
      Se analizó la dependencia con los distintos parámetros de control de la cámara y en base 
      a esto se decidió una configuración de trabajo.
      
      \noindent
      Gracias a esto se caracterizó y se calibró el sensor para la configuración
      utilizada, mediante la implementación de la librería en C++ con filosofía de
      programación orientada a objetos. Utilizando esta librería se observó la presencia 
      los picos de emisión de rayos X $K_{\alpha}$ y $K_{\beta}$ del Cu y
      los picos $K_{\alpha}$ del Fe y del Ca.
      ...
      \noindent
      De forma análoga, se utilizó la librería para la detección de interacciones del sensor con partículas
...
\begin{document}
  ...
  \section{Introducción}\label{sec:intro}
    ...
    a un coste inferior que los CCDs utilizados para determinar la carga depositada en las interacciones sensor-partícula. %FIXME: cambiar detectores de part.
  ...
  \section{Configuración experimental}\label{sec:conf_exp}
    ...
    \subsection{Observación de rayos cósmicos.}\label{sec:conf_exp:cosmic_ray}
      ...
      que es del orden de \SI{100}{\meter^{-2}\second^{-1}}. %FIXME: Me ayudaría poner una referencia.
